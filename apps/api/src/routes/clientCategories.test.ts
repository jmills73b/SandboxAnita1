      ...
      prepare: (sql: string) => {
        ...
        const statement = {
          ...
          first: async <T,>() => {
            ...
            if (sql.includes("UPDATE client_categories SET name")) {
              const [name, id] = boundArgs as [string, number];
            ...
            if (sql.includes("SELECT id FROM client_categories WHERE id = ?")) {
          ...
          run: async () => {
            if (sql.includes("DELETE FROM client_categories")) {
              const [id] = boundArgs as [number];
...
describe("GET /api/client-categories", () => {
  it("rejects a request with no session", async () => {
    const res = await app.request("/api/client-categories", {}, fakeEnv());
  ...
  it("lists categories in sort order", async () => {
    ...
    const res = await app.request(
      ...
      fakeEnv({
        categories: [
          { id: 2, name: "High Net Worth", sort_order: 2 },
          { id: 1, name: "Financial Remedy", sort_order: 1 },
        ],
      }),
    );
...
describe("POST /api/client-categories", () => {
  ...
  it("rejects a duplicate name", async () => {
    ...
    const res = await app.request(
      "/api/client-categories",
      { method: "POST", headers: { Cookie: cookie }, body: JSON.stringify({ name: "Financial Remedy" }) },
      fakeEnv({ categories: [{ id: 1, name: "Financial Remedy", sort_order: 1 }] }),
    );
    expect(res.status).toBe(400);
  });
  ...
  it("creates a new category after the existing ones", async () => {
    ...
    const res = await app.request(
      "/api/client-categories",
      { method: "POST", headers: { Cookie: cookie }, body: JSON.stringify({ name: "Cohabitation" }) },
      fakeEnv({ categories: [{ id: 1, name: "Financial Remedy", sort_order: 1 }] }),
    );
    expect(res.status).toBe(201);
    expect((await res.json()).name).toBe("Cohabitation");
  });
});

describe("PATCH /api/client-categories/:id (rename)", () => {
  it("renames a category", async () => {
    const cookie = await sessionCookie();
    const res = await app.request(
      "/api/client-categories/1",
      { method: "PATCH", headers: { Cookie: cookie }, body: JSON.stringify({ name: "Divorce" }) },
      fakeEnv({ categories: [{ id: 1, name: "Financial Remedy", sort_order: 1 }] }),
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ id: 1, name: "Divorce" });
  });
});

describe("DELETE /api/client-categories/:id", () => {
  it("404s when the category doesn't exist", async () => {
    const cookie = await sessionCookie();
    const res = await app.request(
      "/api/client-categories/99",
