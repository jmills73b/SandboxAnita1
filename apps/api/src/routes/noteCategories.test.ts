      ...
      prepare: (sql: string) => {
        ...
        const statement = {
          ...
          first: async <T,>() => {
            ...
            if (sql.includes("INSERT INTO note_categories")) {
            ...
            if (sql.includes("UPDATE note_categories SET name")) {
              const [name, id] = boundArgs as [string, number];
          ...
          run: async () => {
            if (sql.includes("DELETE FROM note_categories")) {
              const [id] = boundArgs as [number];
...
describe("GET /api/note-categories", () => {
  it("rejects a request with no session", async () => {
    const res = await app.request("/api/note-categories", {}, fakeEnv());
  ...
  it("lists categories in sort order", async () => {
    ...
    const res = await app.request(
      ...
      fakeEnv({
        categories: [
          { id: 2, name: "Advice Given", sort_order: 2 },
          { id: 1, name: "Initial Meeting", sort_order: 1 },
        ],
      }),
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual([
      { id: 1, name: "Initial Meeting" },
      { id: 2, name: "Advice Given" },
    ]);
  });
});

describe("POST /api/note-categories", () => {
  ...
  it("rejects a duplicate name", async () => {
    ...
    const res = await app.request(
      "/api/note-categories",
      { method: "POST", headers: { Cookie: cookie }, body: JSON.stringify({ name: "Initial Meeting" }) },
      fakeEnv({ categories: [{ id: 1, name: "Initial Meeting", sort_order: 1 }] }),
    );
    expect(res.status).toBe(400);
  });
  ...
  it("creates a new category after the existing ones", async () => {
    ...
    const res = await app.request(
      "/api/note-categories",
      { method: "POST", headers: { Cookie: cookie }, body: JSON.stringify({ name: "Follow-up" }) },
      fakeEnv({ categories: [{ id: 1, name: "Initial Meeting", sort_order: 1 }] }),
    );
    expect(res.status).toBe(201);
    expect((await res.json()).name).toBe("Follow-up");
  });
});

describe("PATCH /api/note-categories/:id (rename)", () => {
  it("renames a category", async () => {
    const cookie = await sessionCookie();
    const res = await app.request(
      "/api/note-categories/1",
      { method: "PATCH", headers: { Cookie: cookie }, body: JSON.stringify({ name: "First Meeting" }) },
      fakeEnv({ categories: [{ id: 1, name: "Initial Meeting", sort_order: 1 }] }),
    );
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ id: 1, name: "First Meeting" });
  });
});

describe("DELETE /api/note-categories/:id", () => {
  it("404s when the category doesn't exist", async () => {
    const cookie = await sessionCookie();
    const res = await app.request(
      "/api/note-categories/99",
