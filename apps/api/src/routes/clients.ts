...
async function fetchClient(db: D1Database, id: number): Promise<ReturnType<typeof toClient> | null> {
  const row = await db
    .prepare("SELECT id, name, email, summary, first_invoice_date FROM clients WHERE id = ?")
    .bind(id)
    .first<ClientRow>();
  if (!row) return null;
...
clients.get("/", async (c) => {
  const { results } = await c.env.DB.prepare(
    "SELECT id, name, email, summary, first_invoice_date FROM clients ORDER BY name",
  ).all<ClientRow>();
...
clients.post("/", async (c) => {
  ...
  const created = await c.env.DB.prepare(
    "INSERT INTO clients (name, email, summary) VALUES (?, ?, ?) RETURNING id",
  )
    .bind(trimmed, body.email?.trim() || null, body.summary?.trim() || null)
    .first<{ id: number }>();
...
clients.patch("/:id", async (c) => {
  ...
  await c.env.DB.prepare("UPDATE clients SET name = ?, email = ?, summary = ? WHERE id = ?")
    .bind(name, email, summary, id)
    .run();
